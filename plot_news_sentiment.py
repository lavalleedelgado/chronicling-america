...
def plot_news_sentiment(log_path, base_path, base_name, base_scalar=1):
    ...
    base_scalar (float): factor by which to scale base comparison data.
    ...
    news_sentiment = _prepare_news_sentiment(log)
    news_volume = _prepare_news_volume(log, news_sentiment)
    news_y_min, news_y_max = _calculate_axis_range(news_volume["w_volume"])
    negative_polarity = _prepare_negative_polarity(news_sentiment)
    base_comparison = _prepare_base_comparison(base_path, base_scalar)
    base_y_min, base_y_max = _calculate_axis_range(base_comparison["count"])
    ...
    figure_a.add_layout(LinearAxis(y_range_name="base_comparison", \
        axis_label=base_name + " by " + str(base_scalar) + "x"), "right")
...
def _prepare_news_sentiment(log):
...
def _prepare_news_volume(log, sentiment):
    ...
    log (DataFrame): Chronicling America log.
    sentiment (DataFrame): news sentiment data from _prepare_news_sentiment().
    ...
    volume["weight"] = volume["year"].apply(_lookup_weight, \
        args=(period_weights,))
...
def _lookup_weight(year, period_weights):
...
def _calculate_axis_range(series):
...
def _prepare_negative_polarity(sentiment):
...
def _prepare_base_comparison(base_path, base_scalar):

    '''
    Prepare base comparison data and scale its range to one more appropriate
    for the plots. Again, this file must have "year" and "count" columns.

    base_path (str): path to base comparison CSV data.
    base_scalar (float): factor by which to scale base comparison data.

    Return base comparison data (DataFrame).

    '''

    base_comparison = pd.read_csv(base_path)
    base_comparison["count"] = base_comparison["count"] / base_scalar
    return base_comparison[["year", "count"]]
...
def run():
    ...
    if len(arguments) < 2:
        print("Expected at least four arguments: path to Chronicling America "
              "log CSV, path to annual base comparison CSV, \nbase scalar "
              "by which to adjust its axis (or 1), and base name.")
        sys.exit()
    log_path = arguments.pop(0)
    base_path = arguments.pop(0)
    try:
        base_scalar = float(arguments.pop(0))
        assert(base_scalar > 0)
    except TypeError:
        print("Expected positive nonzero integer for base scalar.")
        sys.exit()
    base_name = " ".join(arguments)
    plot_news_sentiment(log_path, base_path, base_name, base_scalar)
    print("Finished!")
    sys.exit()
